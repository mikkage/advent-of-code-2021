...
loop do
  ...
  # If there are no spots with more than 0 energy, then we've found the first iteration
  # where all spots are 0 energy and can break from the loop
  break if grid.select { |k, v| v[:energy] > 0 }.length == 0
end
